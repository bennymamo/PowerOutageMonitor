...
The development branch adds a dedicated, searchable device dashboard with expandable grid, solar, battery, home-load and equipment sections. Unknown fields remain available with their original names; private identifiers and credentials are hidden. It also adds **Request PowerOcean readings**, a documented read-only `POST` request for phase voltage, solar strings and core power readings, to try when the broad inspection is denied. These additions are not in the first published preview APK. If EcoFlow denies both methods, use **Support** in its developer console to request read-only PowerOcean API access; this is not an Android permission problem. Never send API keys in support messages.

Source builds additionally offer **Settings → Power sources → PowerOcean account**, an optional experimental connection using your normal EcoFlow email/password and inverter serial. It supports model selection, Europe/US account regions, encrypted credential storage and inclusion in password-protected Power sources backups. Save the connection, then tap **Connect and read device** for a searchable dashboard, including individual batteries, meter readings and raw system states when reported. Auto-refresh while viewing is optional, with a configurable 10–60 second request interval; it pauses in the background. These additions are not in the first published preview APK.

**Account-access caution:** This experimental interface is unofficial. EcoFlow has not confirmed its permitted request rates or authorized this integration; reduced traffic cannot guarantee protection from account restrictions. Its [Managing System terms](https://account.ecoflow.com/agreement/jt/en-us/TermsOfUse.html) restrict automated access and describe assistance with API accounts and associated power stations. Request approved read-only PowerOcean access and applicable limits from EcoFlow before continuous use. Account monitoring remains unavailable as a selectable source. Manual MQTT inspections issue two reading requests once per minute; optional live-report activation adds one request every 20 seconds for the selected inspection duration. These intervals are app choices, not manufacturer-approved quotas.

After connecting, **Inspect live push feed** tries a secure account MQTT connection. Choose **45 seconds** for an access check, **5 minutes** for a short comparison, or **15 minutes · grid test** for a manual grid-loss/recovery test with time to reach the switch and wait for reconnection. The app tries the mobile-account connection route first, then a portal route if needed; MQTT credentials remain in memory. If readings only update while EcoFlow's app is open, enable **Request live reporting** before starting: this sends the portal's temporary reporting request every 20 seconds during inspection. It stops requesting when inspection ends and sends no charging, reserve or output controls. Leave it off for reading requests alone. The inspection decodes identified PowerOcean phase, solar, battery, grid-state and energy reports, including masked binary messages, JSON reports and an alternate system-total report used by some Single Phase installations. It counts unsupported/retained packets and shows report receipt ages. Actual device compatibility and unattended freshness are still being checked; a connection alone does not establish working outage detection. Close EcoFlow's app and portal when checking whether readings continue independently. The account protocol and report mappings draw on [niltrip/powerocean](https://github.com/niltrip/powerocean) and [shuette42/ecoflow-energy-ha](https://github.com/shuette42/ecoflow-energy-ha); attribution and licenses are in [Third-party notices](THIRD_PARTY_NOTICES.md).

The live dashboard includes a **Grid comparison** panel with the last explicitly reported grid code and a short timeline of code changes. Retained values and reports without a grid code cannot refresh this comparison. Codes are shown as observations, without assuming their meaning across different models. Backup power can keep AC voltage present during a utility outage, and the inverter may report reconnection several minutes after the utility supply returns. Use only an existing mains switch you know how to operate; keep the inverter and the internet connection powered, and compare normal supply, backup operation and eventual reconnection. Do not change inverter settings to run this test.

Cloud monitoring remains a preview rather than a selectable outage source. Some PowerOcean cloud values have been reported to stop refreshing when the EcoFlow app and web portal are closed. A successful request is therefore not yet sufficient proof that its data is current. Activation will require a controlled grid-loss and restoration test that proves the selected system's grid observations remain fresh unattended. Until then, uncertain cloud data cannot enter the outage state machine. Zero grid power flow does not mean the grid is down; raw state codes are deliberately not guessed.
