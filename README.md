...
3. Live-feed inspections automatically request reporting every 20 seconds during the inspection. Charger-first assistance automatically requests reporting once per scheduled/manual check. Outside charger-first mode, **Continuous background live reporting** is an optional 20-second request loop. These requests change no charging, reserve or output controls; manufacturer permission and quotas remain unconfirmed.
...
5. Run a fresh live-feed test with that profile enabled. Then choose **Use PowerOcean for monitoring** (or **Use charger + EcoFlow assistance**), and start it with the Status dashboard master switch. The activation card explains any missing requirement. A successful HTTP connection alone may return cached data and does not complete the live test; activate within 90 seconds of its latest successful evidence. If the test expires, run another inspection. **Already tested this installation?** Enable **Use my previous successful test** on the activation card to skip that setup live check. It accepts the saved verified profile for the same inverter/model/region; it does not make old readings current or bypass runtime evidence checks. If the profile has not been recorded yet, **I already tested this installation** lets you acknowledge your earlier physical test without repeating the grid cut. Missing, retained, unsupported or stale grid evidence remains **Unknown**.
...
**Charger-first assistance (1.0.2):** Under **Verify and monitor**, enable **Charger-first assistance** and select **Use charger + EcoFlow assistance** after a live test or your explicit **Use my previous successful test** choice. An optional mode keeps local charger outage alerts working even when EcoFlow is unreachable. Normal EcoFlow requests default to once per hour; charger loss switches requests to once per minute. Both schedules support manual-only checks or intervals from five seconds to 24 hours. Faster settings create more traffic and are not approved manufacturer quotas. The faster schedule starts with charger loss or an EcoFlow-reported possible outage. It remains active while the charger is disconnected or an EcoFlow-detected grid episode awaits full reconnection, including after meter evidence indicates grid return. It returns to normal after charger reconnection and restoration. A manual **Check EcoFlow now** action is available while monitoring runs. It shows progress, a 45-second timeout, and whether live reports arrived or grid evidence was verified. Expand **EcoFlow readings** for the reported grid code and Meter 1 reading, short explanations and their individual phone receipt times. Labels distinguish live device pushes from possibly cached request replies and readings received before this check. That section also shows check times and additional reported power values. Connection details and tappable alert/Internet shortcuts sit under **Connections & alerts** to keep Status concise. Live power reports can arrive while grid evidence remains incomplete; charger detection keeps working. Each scheduled/manual check automatically requests the live push feed; it does not run a separate 20-second activation loop between checks. This option is off by default. A charger that loses power with the grid gives the earliest local alert. A verified EcoFlow grid-and-meter outage also counts if a UPS keeps the charger powered; charger power cannot veto it. If the charger stays powered, detection depends on the next normal EcoFlow check—up to an hour with the default schedule. Missing EcoFlow readings after such an outage become Unknown; they cannot invent restoration. Updated recovery evidence is required. EcoFlow-only mode retains its optional charger-loss corroboration setting; assistance mode allows either source to detect loss independently. An old connected report received before charger loss cannot clear that outage. It uses the same device-push grid/meter comparison as the live test, with fewer requests. Request replies can be cached; packet receipt is not a verified measurement timestamp.

**Checking for stuck readings:** Each EcoFlow check needs a new, non-retained device update received after that check began; replies alone cannot verify that the feed is current. During normal monitoring, an initial connected grid code from a reply can be used after a newer device update arrives. The unchanged code remains valid while device updates continue. A gap longer than 90 seconds makes the evidence Unknown. Replies cannot overwrite an established grid state or clear an ongoing outage. The app compares meaningful power values across separate checks, ignoring request IDs and timestamps. Three consecutive checks with identical power values show a dashboard warning and, by default, send one warning per episode through every enabled alert method. A genuinely steady load can also produce identical values, so this is a possible data problem, not an outage confirmation. Changing power values clears the warning. Under **Verify and monitor → Stuck-reading safeguards**, you can turn warning delivery off or enable **Ignore stuck EcoFlow readings**: checks continue, but those readings cannot confirm an outage or recovery until power values change. Charger-first local detection continues independently.

**Dashboard shortcuts:** Tap the Grid source, Source reading, Alert channels, Internet or EcoFlow checks rows (marked with a small chevron) to open their relevant settings or diagnostics directly. These shortcuts do not change monitoring settings.

**Temporarily pause assistance:** In charger-first mode, use **Pause EcoFlow assistance** on Status to stop scheduled EcoFlow requests and exclude its evidence while charger monitoring continues. **Resume EcoFlow assistance** returns to the configured schedule without signing in again if the current session is still valid. Pause does not erase saved credentials; a currently connected feed can still receive unsolicited device pushes and send keepalives. This temporary pause is not imported from a backup. Automatic exclusion differs from Pause: it keeps requesting readings so changing values can resume assistance automatically.

**Traffic and account access:** Sessions and broker credentials are reused during monitoring and network reconnects; scheduled checks do not log in again. The secure connection can receive unsolicited device updates and send keepalives between requests. An hourly interval controls how often the app requests readings; it does not restrict incoming updates to hourly. The dashboard shows **Last check requested** separately from **Last device update received**. In charger-first mode, each check always sends one temporary live-report activation and one reading request, even if the continuous-mode live-report option is off. EcoFlow’s own app does not need to stay open. This does not guarantee that every returned field is a new equipment measurement. In continuous integration mode, background monitoring requests readings once per minute by default; the interval is configurable from 60 to 3,600 seconds. Optional live reporting adds a request every 20 seconds. These are app choices, **not EcoFlow-approved quotas**. Higher intervals can let evidence expire to Unknown. Network failures use increasing reconnect delays; explicit authentication/access rejection and HTTP 429 stop automatic access attempts until you reconnect manually. Turning monitoring off stops the feed and removes its notification.
...
After restoring an account backup, the app starts with the Android charger source. Explicitly select PowerOcean again after checking the restored setup. Run a fresh live test, or use the saved **Use my previous successful test** option for an already-verified installation. The saved profile choice, optional live-report setting, charger-confirmation setting and charger-first schedules, stuck-reading safeguard settings and the previous-test choice are included when **Power sources** is backed up. Recovery evidence and live sessions are not imported; they must be established on the restored device.
