...
The app now has a four-step first-run wizard, Status, History and grouped Settings screens, plus Diagnostics and an isolated Test mode under Settings. The wizard explains detection, battery safety and background operation, collects the device name and stable timing defaults, then enables monitoring and requests notification permission. A user-controlled master switch starts or stops the foreground monitor, its alarms and its ongoing notification. The service observes Android's external-power state without polling, persists the outage state before first unlock, resumes after reboot or app upgrade, and records completed outages or brief interruptions locally. Confirmed outage and stable-restoration events flow through a provider-independent durable queue to enabled Telegram chats with per-recipient retry and de-duplication.
