...
The app now has Status, History and grouped Settings screens, plus Diagnostics and an isolated Test mode under Settings. A user-controlled master switch starts or stops the foreground monitor, its alarms and its ongoing notification. The service observes Android's external-power state without polling, persists the outage state before first unlock, resumes after reboot or app upgrade, and records completed outages or brief interruptions locally. The provider-independent alert queue and retry rules are implemented and tested. Telegram setup, encrypted credential storage, chat discovery and direct test delivery are implemented; automatic outage delivery is the next wiring step.
