...
internal fun SmsSetupScreen(
    ...
    Column(
        ...
        SetupFlowFooter(setupSteps, setupStep, helpLevel.isGuided, loading, { setupStep = it }, onBack, finishEnabled = config.recipients.isNotEmpty() && enabled == config.enabled && validateSmsRecipients(recipientText).recipients == config.recipients, nextEnabled = when (setupStep) { 0 -> capability.supported && capability.permissionGranted; 1 -> validateSmsRecipients(recipientText).error == null; else -> true })
