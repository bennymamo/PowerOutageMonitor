...
internal object PowerOceanLossConfirmation {
    enum class Reason { UNKNOWN, CONNECTED, RETURN_PENDING, ECOFLOW_AND_METER, CHARGER_CORROBORATED, WAITING_FOR_CHARGER }
    data class Result(val availability: GridAvailability, val reason: Reason)

    fun evaluate(grid: PowerOceanGridCorrelation.Snapshot, chargerExternallyPowered: Boolean?, requireCharger: Boolean): Result =
        when (grid.state) {
