...
4. Use the assigned first preview version `1.0.0-preview.1` (build code `4`), then build and verify its signed APK and SHA-256 checksum. Later updates must increase the build code.
...
7. Re-test alarm dismissal, charger transitions and provider delivery on the release build. An unattended physical monitoring window is recommended; the owner deferred the additional signed overnight run to post-install testing for this preview.
8. Create a GitHub **pre-release** with a version tag, concise user notes, the signed APK and its SHA-256 checksum. Only then update the README's installation section. README download, release-note and checksum links must always point to `https://github.com/bennymamo/PowerOutageMonitor/releases`, never to a specific release tag or asset; users choose a version and its files there.

The signing key is part of the app's update identity: every later APK installed over the original must use the same key. The permanent key was created after the owner chose Bitwarden Premium for an encrypted file attachment and password storage. The owner saved the Note and attached the keystore, downloaded the attachment, and built using independently entered Bitwarden passwords. The resulting APK matches the permanent signing certificate. Windows PowerShell 5.1 initially treated a Java warning as a terminating verification error; the verifier now captures native output and explicitly checks the exit code and certificate. Valid, invalid-APK and wrong-certificate checks passed on Windows PowerShell 5.1. Development/debug APKs must never be published as release assets.
...
Physical recovery milestone: the Samsung test device completed its one-time debug-to-permanent-signer migration. The owner created and unlocked an all-category recovery archive before reinstalling; its PC copy was checksum-verified. After fresh signed installation, the owner confirmed settings, Telegram configuration and History restored successfully. Permanently signed build code 3 installed in place over build code 2, retaining the original installation timestamp; build code 4 also updated in place. Post-update History, live Telegram test/outage/restoration delivery, built-in and custom sound previews, immediate alarm dismissal and master-switch notification checks passed. The owner separately confirmed build 4's Stop sound notification action works on the Samsung. No additional signed overnight observation is claimed; it was deferred to post-install testing.
